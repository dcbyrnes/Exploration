...
featurizer = sklearn.pipeline.FeatureUnion([
    ("rbf1", RBFSampler(gamma=5.0, n_components=4)),
    ("rbf2", RBFSampler(gamma=2.0, n_components=4)),
    ("rbf3", RBFSampler(gamma=1.0, n_components=4)),
    ("rbf4", RBFSampler(gamma=0.5, n_components=4))
])
...
class PolicyEstimator:
    ...
    def _build_model(self):
        self.state = tf.placeholder(tf.float32, [16], name="state")
...
if __name__ == "__main__":
    ...
    reward_sequence = actor_critic(episodes=100, gamma=gamma, display=False, lamb=lamb, policy_lr=policy_lr, value_lr=value_lr)
    np.save(reward_totals, '')
