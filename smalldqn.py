...
class LinearNet(nn.Module):
    def __init__(self):
        ...
        self.lin = nn.Linear(25, 4)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        x = x.permute(0, 3, 1, 2)
        x = x[:,:1,:,:].contiguous()
        x = self.lin(x.view(x.size(0), 25))
        x = self.relu(x)
        return x.view(x.size(0), -1)
...
env = gym.make('small-maze-1-v0')
# memory = ReplayMemory(size=2000, shape=(5,5,8))
...
# optimizer = Adam(model.lin.parameters())
...
for i_env, env in enumerate([gym.make('small-maze-{}-v0'.format(i)) for i in range(10)]):
    memory = ReplayMemory(size=2000, shape=(5,5,8))
    core = CoreNet()
    # core.load_state_dict(torch.load('teststate'))
    # model = HeadNet(core)
    ...
    for i_episode in range(num_episodes):
        ...
        if (steps < 10):
            count_solved += 1
            # if count_solved > 2:
            print('Solved {} in {} steps'.format(i_env, i_episode))
            break
        else:
            count_solved = 0

    # print('Episode ', i_episode, ' Steps ', steps)
