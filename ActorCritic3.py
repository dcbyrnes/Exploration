...
class PolicyEstimator:
    def __init__(self, env, lamb=1e-5, learning_rate=0.01, scope="policy_estimator", state_shape=8, action_dim=2):
        self.env = env
        self.lamb = lamb
    ...
    def _build_model(self):
        self.state = tf.placeholder(tf.float32, [self.state_shape], name="state")
...
class ValueEstimator:
    def __init__(self, env, learning_rate=0.01, scope="value_estimator", state_shape=8, action_dim=2):
        self.env = env
        self.learning_rate = learning_rate
        self.state_shape = state_shape
        self.action_dim = action_dim
...
if __name__ == "__main__":
    ...
    loss = actor_critic(episodes=200, gamma=gamma, display=False, lamb=lamb, policy_lr=policy_lr, value_lr=value_lr)
    print(-loss)
    env.close()